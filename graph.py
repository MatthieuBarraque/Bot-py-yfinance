...
async def stock(ctx, arg):
    ...
    if arg.lower() not in stocks:
        await ctx.send(
            "L'action rentrez en parametre n'est pas enregistrez dans notre BDD"
        )
        await ctx.send("La commande est : /graph <action>")
    ...
    try:
        if arg.lower() in stocks:
            ...
            )  # 20 jours ouvrés, cela correspond à un mois
            hist["MA50"] = (
                hist["Close"].rolling(window=50).mean()
            )  # 50 jours ouvrés, cela correspond à 2 mois et demi j'utilise
            ...
            ax.bar(
                ...
                bottom=hist["Open"],
                color=np.where(hist["Close"] - hist["Open"] > 0, "g", "r"),
            )
            ...
            embed = discord.Embed(
                title=f"Informations sur {arg.capitalize()}", color=0x3498DB
            )

            # Utilisation du formatage Markdown pour le style
            embed.add_field(
                name="**Cours actuel**", value=f"**{stock_value:.2f} €**", inline=True
            )
            embed.add_field(
            ...
            embed.add_field(
                name="📈 Moyenne mobile sur 20 jours",
                value=f'{hist["MA20"][-1]:.2f} €',
                inline=True,
            )
            embed.add_field(
                name="📉 Moyenne mobile sur 50 jours",
                value=f'{hist["MA50"][-1]:.2f} €',
                inline=True,
            )
            embed.add_field(
            ...
            embed.add_field(
                ...
                value=f'{hist["Low"].min():.2f} €',
                inline=True,
            )
            embed.add_field(
                name="🔄 Volume moyen",
                value=f'{hist["Volume"].mean():.0f}',
                inline=True,
            )  # Pas de symbole € pour le volume
